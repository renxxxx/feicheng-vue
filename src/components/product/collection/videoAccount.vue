<template>
	<div class="videoAccount">
		<div style="text-align: right;height: 20px;line-height: 20px;align-items: center;color: #787a7a;"></div>
		<div class="search">
		...
		<div class="searchList" v-infinite-scroll="nextPage" :infinite-scroll-disabled="load" infinite-scroll-distance="10">
			<el-table align='center' :data="userList"  style="width: 100%"   @row-click="detailsFn">
				<el-table-column   prop="date" label="视频号" min-width="43%">
			 ...
			 <el-table-column align='center'   prop="name" min-width="13%" >
			   ...
			   <el-table-column align='center'   prop="name" min-width="10%">
				...
				<el-table-column align='center'   prop="name" min-width="8%">
					...
					<template slot-scope="scope">
						<span style="color: #e8edee;font-size: 14px;text-align: center;"  @click.prevent.stop="deteleFn(scope.row)">删除</span>
					</template>
				</el-table-column>
			 </el-table>
			<!-- </div> -->
		...
		deteleFn(value){
			...
			if(value.wxVideoaccountId){
				...
				.then(res=>{
					...
					if(res.data.code == 0){
						let num = this.userList.findIndex(n=>value.wxVideoaccountId == n.wxVideoaccountId);
