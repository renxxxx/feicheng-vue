	...
	<div class="douyinSearch" style="margin-top:24px;">
		...
		<div class="searchList" v-infinite-scroll="nextPage" :infinite-scroll-disabled="load" infinite-scroll-distance="10">
			<el-row style="margin-top: 15px;min-width: 500px;">
				<div class="_1oRkJsku" v-for="(item,inx) in videoList" :key="inx" @click.prevent="playFn(item.video)">
					<!-- <a :href="item.video" target="_blank"> -->
						...
						<div class="listTitle">
							...
							<!-- <router-link :to="{path:'/searchDetails/searchDetails_index',query:{data:item.wxVideoaccountId}}"  target="_blank"> -->
								<div class="listTitle_xia" @click.stop="nameDetalisFn(item.wxVideoaccountId)">
...
<script>
	...
	export default {
		...
		methods: {
			nameDetalisFn(_value){
				window.open('/#/searchDetails/searchDetails_index?data='+_value, '_blank');
			},
			nextPage() {
				this.page++;
				this.getData();
